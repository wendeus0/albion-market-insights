...
export const ITEM_CATALOG: Record<CatalogCategoryKey, CatalogCategory> = {
  ...
  spears: {
    label: "Spears",
    ids: genIds(["MAIN_SPEAR", "2H_SPEAR", "2H_PIKE", "2H_LANCE", "2H_GLAIVE"]),
  },
  daggers: {
    label: "Daggers",
    ids: genIds([
...
export const ITEM_IDS: string[] = Object.values(ITEM_CATALOG).flatMap(
  (c) => c.ids,
);
...
export const ITEM_NAMES: Record<string, string> = Object.fromEntries(
  ITEM_IDS.map((id) => {
    ...
    if (label) return [id, `${tierPrefix} ${label}`.trim()];

    const fallback = type
      .replace(/_/g, " ")
      .replace(/\b(\w)/g, (_, c: string) => c.toUpperCase());

    return [id, `${tierPrefix} ${fallback}`.trim()];
  }),
);

export function formatTierBadge(itemId: string, tier: string): string {
  const enchantMatch = itemId.match(/@([0-3])$/);
  const enchantLevel = enchantMatch ? parseInt(enchantMatch[1], 10) : 0;
  return enchantLevel > 0 ? `${tier}.${enchantLevel}` : tier;
}
