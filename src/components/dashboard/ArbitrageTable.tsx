...
export function ArbitrageTable({ items, className }: ArbitrageTableProps) {
  ...
  const filteredItems = useMemo(() => {
    ...
    const result = items.filter((item) => {
      ...
      const matchesNetProfit =
        minNetProfitValue === null || item.netProfit >= minNetProfitValue;
      const matchesRoi =
        minRoiValue === null || item.netProfitPercent >= minRoiValue;
            ...
            {paginatedItems.length === 0 ? (
              ...
              paginatedItems.map((item) => (
                ...
                >
                  ...
                  <td className="p-3 text-right">
                    <span className="font-mono text-sm font-semibold text-success">
                      +{item.netProfitPercent.toFixed(1)}%
                    </span>
                  </td>
      ...
      {totalPages > 1 && (
        <div className="p-4 border-t border-border/50 flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            ...
            {filteredItems.length} opportunities
          </p>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
