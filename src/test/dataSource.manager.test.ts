...
describe('dataSource.manager', () => {
  ...
  describe('state transitions', () => {
    ...
    it('should update lastSuccessfulFetch on setReal', () => {
      ...
      const status = dataSourceManager.getStatus();
      expect(status.lastSuccessfulFetch!.getTime()).toBeGreaterThanOrEqual(before.getTime());
      expect(status.lastSuccessfulFetch!.getTime()).toBeLessThanOrEqual(after.getTime());
    });
  });
});
