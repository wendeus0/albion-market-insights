---
name: next_steps
description: Próximos passos recomendados para o projeto — sobrescrito a cada sessão
type: project
---

## Next recommended steps

1. **[AÇÃO DO USUÁRIO] Corrigir Pages dashboard**: atualizar `VITE_USE_REAL_API` de `false` para `true` em Cloudflare Pages → Settings → Environment variables → Production → Retry deployment
2. **Verificar produção**: após redeploy, confirmar que o dashboard mostra dados reais (badge "Live" + dados de preço)
3. **Abrir SPEC da próxima frente**: iniciar novo ciclo `implement-feature` com escopo definido pelo usuário
4. **Manter observação Node 24**: continuar janela de estabilidade antes de promover para default
5. **Reavaliar issue #59 (flakiness)**: decidir se investigação adicional entra no próximo ciclo
6. **Frentes C/D do Contrato de Autonomia**: permanecem bloqueadas pelas issues #65 e #66
